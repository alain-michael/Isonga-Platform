...
const AdminInvestors: React.FC = () => {
  ...
  const filteredPartners = partners.filter((partner) => {
    const matchesSearch =
      ...
      partner.investor_type.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (partner.partner_name &&
        partner.partner_name.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesType =
      typeFilter === "all" || partner.investor_type === typeFilter;
    const matchesStatus =
      statusFilter === "all" ||
  ...
  const stats = {
    ...
    active: partners.filter((i) => i.is_active).length,
    inactive: partners.filter((i) => !i.is_active).length,
    totalInvestment: partners.reduce(
      (sum, i) =>
        sum +
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div>
            ...
            <p className="text-lg text-neutral-600 dark:text-neutral-400">
              Onboard and manage partner profiles and criteria
            </p>
          </div>
      ...
      <div className="glass-effect p-4 rounded-2xl glass-effect dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 mb-6">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search partners by name, email, or type..."
              value={searchTerm}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPartners.map((partner) => (
            <div
              ...
              onClick={() => navigate(`/admin/investors/${partner.id}`)}
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                      {investorTypeDisplay[partner.investor_type] ||
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="pt-2 border-t border-neutral-100 dark:border-neutral-700">
                  ...
                  <p className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 mt-1 ml-6">
                    {formatCurrency(partner.min_investment)} -{" "}
                    {formatCurrency(partner.max_investment)}
                  </p>
                </div>
              </div>
                ...
                {filteredPartners.map((partner) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-neutral-500 dark:text-neutral-400">
                        to {formatCurrency(partner.max_investment)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/admin/investors/${partner.id}`);
                        }}
